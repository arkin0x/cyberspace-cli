...
class TestMovePlanes(unittest.TestCase):
    ...
    def test_move_toward_allows_plane_switch_at_end(self) -> None:
        old_home = os.environ.get("CYBERSPACE_HOME")
        try:
            with self._with_tmp_home() as td:
                os.environ["CYBERSPACE_HOME"] = td
                ...
                # Start at (100,200,300) plane=0 and move toward (101,200,300) plane=1.
                c0 = xyz_to_coord(100, 200, 300, plane=0).to_bytes(32, "big").hex()
                ...
                buf = io.StringIO()
                with contextlib.redirect_stdout(buf), contextlib.redirect_stderr(buf):
                    move(to=None, by=None, toward=f"0x{c_target}", max_lca_height=20, max_hops=0)

                st = load_state()
                assert st is not None
                self.assertEqual(st.coord_hex, c_target)
                # One hop for xyz, one hop for plane switch.
